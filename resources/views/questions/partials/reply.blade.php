    ...
    <div class="nested-replies ml-4">
        @foreach($replies as $reply)
            <div class="bg-gray-200 p-2 mt-2 rounded-lg">
                ...
                @include('questions.partials.reply', ['replies' => $reply->replies])


                <!-- Reply form for this reply -->
                <div id="reply-form-{{ $reply->id }}" class="mt-2">
                    <form action="{{ route('comments.reply', ['comment' => $reply->id]) }}" method="post">
                        @csrf
                        <input type="hidden" name="answer_id" value="{{ $answer->id }}">
                        <input type="hidden" name="parent_id" value="{{ $reply->id }}">
                        <textarea name="content" class="w-full mt-2 p-2 bg-gray-700 border border-gray-300 rounded-lg"
