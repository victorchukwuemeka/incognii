...
@section('content')
<div class="bg-gray-900 text-white min-h-screen flex flex-col justify-center pt-0 items-center px-4">
    <div class="max-w-lg w-full bg-gray-800 rounded-md p-4 mb-4">
        <p class="text-lg">Q : {{ $question->your_question }}?</p>
    </div>
    <div class="max-w-lg h-full w-full bg-gray-800 rounded-md p-4 mb-4">
        <button id="showAnswerForm"
        class="btn rounded-full bg-gray-500 hover:bg-gray-700 text-white font-bold py-2 px-4 ">
      ...
      <form id="answerForm" method="POST" action="{{ route('answer.store') }}"
        ...
        <div class="border border-gray-300 bg-gray-300  shadow-md rounded-lg p-6 mt-4">
            <p class="text-gray-800">A : {{ $answer->answer }}</p>
            <form class="hidden" action="{{ route('comments.store') }}" method="post">
              @csrf
